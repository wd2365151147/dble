...
public class XACommitFailStage extends XACommitStage {
    ...
    public XAStage next(boolean isFail, String errMsg, byte[] errPacket) {
        ...
        if (!isFail || xaOldThreadIds.isEmpty()) {
            ...
            XASessionCheck.getInstance().getCommittingSession().remove(session.getFrontConnection().getId());
            // resolve alert
        ...
        if (!session.closed()) {
            session.getFrontConnection().close("COMMIT FAILED but it will try to COMMIT repeatedly in background until it is success!");
        }
