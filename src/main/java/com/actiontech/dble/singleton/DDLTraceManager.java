...
public final class DDLTraceManager {
    ...
    private final Map<MySQLShardingService, DDLTraceInfo> traceMap = new ConcurrentHashMap<>();

    private DDLTraceManager() {
    }


    public void startDDL(MySQLShardingService service) {
    ...
    public void updateRouteNodeStatus(MySQLShardingService service, RouteResultsetNode routeNode, DDLTraceInfo.DDLConnectionStatus status) {
        DDLTraceInfo info = traceMap.get(service);
        if (info != null) {
    ...
    public void updateDDLStatus(DDLTraceInfo.DDLStage stage, MySQLShardingService service) {
        DDLTraceInfo info = traceMap.get(service);
        if (info != null) {
            info.setStage(stage);
    ...
    public void endDDL(MySQLShardingService service, String reason) {
        DDLTraceInfo info = traceMap.get(service);
        if (info != null) {
    ...
    public void printDDLOutOfLimit() {
        for (Map.Entry<MySQLShardingService, DDLTraceInfo> entry : traceMap.entrySet()) {
