...
public class MultiTablesMetaJob implements ResponseHandler, Runnable {
    ...
    @Override
    public void connectionAcquired(final BackendConnection con) {
        logger.info("connectionAcquired on connection " + con);
        con.getBackendService().setResponseHandler(this);
        con.getBackendService().setComplexQuery(true);
        try {
            con.getBackendService().query(sql, true);
            connection = con;
